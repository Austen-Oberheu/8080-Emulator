...
State8080* Init8080(void)
{
	State8080* state = static_cast<State8080*>(calloc(1, sizeof(State8080)));
	state->memory = static_cast<uint8_t*>(malloc(0x10000));  //16K
	return state;
}
...
int Disassemble8080Op(unsigned char *codebuffer, int pc) {
	...
	case 0x36: printf("MIV M, #0x%02x", code[1]); opbytes = 2; break;
	...
	case 0xe4: printf("COP $%02x%02x", code[2], code[1]); opbytes = 3; break;
	...
	default: printf("undefined instruction"); break;
	}
...
void UnimplementedInstruction(State8080* state) {
	//Program counter will have advanced one so that needs to be undone
